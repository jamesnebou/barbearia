...
export const metadata = { title: "Lojinha | NexaWi Barbearias" };
...
export default async function ProdutosPage({ searchParams }) {
  ...
  const podeConfigurarLojinha = ["owner", "gerente"].includes(membership?.papel);
    ...
    <main className="px-5 py-8 sm:px-8 lg:px-10">
      <section className="mx-auto max-w-7xl">
        <PageHeader
          ...
          action={(
            <a href={lojinhaAtiva ? `/c/${activeClinic.slug}#loja` : `/c/${activeClinic.slug}`} target="_blank" rel="noreferrer" className="dashboard-primary-button inline-flex h-10 items-center gap-2 rounded-lg px-4 text-sm font-semibold text-white">
        ...
        <div className={`mt-7 flex flex-col gap-4 rounded-xl border p-5 sm:flex-row sm:items-center sm:justify-between ${lojinhaAtiva ? "border-emerald-200 bg-emerald-50/80" : "border-neutral-200 bg-neutral-100/80"}`}>
          ...
          {podeConfigurarLojinha ? (
            ...
            <span className="text-xs font-semibold text-neutral-500">Somente proprietário ou gerente pode alterar.</span>
          )}
        </div>
          ...
          <details className="product-edit-panel mt-5 rounded-xl border border-white/10 p-5">
            ...
            <form action={updateStoreCommerceSettingsAction} className="mt-5 grid gap-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <label className="block"><span className="text-sm font-medium text-neutral-200">Responsável pela entrega</span><select name="entrega_modo" defaultValue={storeConfig.entregaModo} className="dashboard-field mt-2 h-11 w-full rounded-lg px-3 text-sm"><option value="propria">Equipe da barbearia</option><option value="motoboy">Motoboy contratado</option></select></label>
              ...
              <p className="text-xs leading-5 text-neutral-400">A entrega é operacional: o sistema cobra a taxa e registra o endereço, mas a barbearia chama e acompanha o motoboy por conta própria.</p>
