...
export const metadata = { title: "Pedidos da Lojinha | NexaWi Barbearias" };
...
export default async function StoreOrdersPage({ searchParams }) {
  ...
  const [ordersResult, cartsResult, couponsResult] = await Promise.all([
    supabase.from("barbearia_pedidos").select("id, numero, token_publico, cliente_id, status, pagamento_status, entrega_tipo, nome_cliente, telefone_cliente, email_cliente, subtotal, desconto, frete, total, cupom_codigo, forma_pagamento, asaas_payment_id, invoice_url, pago_em, expiracao_reserva_em, observacoes, created_at, itens:barbearia_pedido_itens(id, nome_produto, sku, quantidade, valor_unitario, total)").eq("barbearia_id", activeClinic.id).order("created_at", { ascending: false }).limit(120),
    supabase.from("barbearia_carrinhos_abandonados").select("id, token_recuperacao, status, nome, telefone, email, subtotal, itens, ultima_interacao_em, lembrete_enviado_em, quantidade_lembretes, consentimento_recuperacao").eq("barbearia_id", activeClinic.id).in("status", ["ativo", "recuperado"]).order("ultima_interacao_em", { ascending: false }).limit(40),
    supabase.from("barbearia_cupons").select("id, codigo, descricao, tipo, valor, pedido_minimo, desconto_maximo, limite_usos, usos, inicia_em, termina_em, ativo").eq("barbearia_id", activeClinic.id).order("created_at", { ascending: false }),
  ]);
