...
async function getContext(managerOnly = false) {
  ...
  if (managerOnly && !["owner", "gerente"].includes(membership?.papel)) {
    redirectMessage("erro", "Somente proprietário ou gerente pode realizar esta ação.");
  }
...
export async function updateStoreOrderStatusAction(formData) {
  ...
  const { data: order, error } = await supabaseAdmin.from("barbearia_pedidos").select("id, pagamento_status, status").eq("id", id).eq("barbearia_id", clinicId).maybeSingle();
...
export async function confirmPickupPaymentAction(formData) {
  const { clinicId } = await getContext();
  const id = text(formData, "id", 80);
  const { data: order, error } = await supabaseAdmin.from("barbearia_pedidos").select("id, cliente_id, total, pagamento_status").eq("id", id).eq("barbearia_id", clinicId).maybeSingle();
  if (error) throw error;
  if (!order) redirectMessage("erro", "Pedido não encontrado.");
...
export async function requestStoreOrderRefundAction(formData) {
  ...
  const { data: integration } = await supabaseAdmin.from("barbearia_integracoes").select("barbearia_id, ativo, configuracao_publica, segredos_criptografados").eq("barbearia_id", clinicId).eq("provedor", "asaas").eq("ativo", true).maybeSingle();
  const secrets = integration ? decryptBarbeariaSecrets(integration.segredos_criptografados) : {};
  if (!secrets.apiKey) redirectMessage("erro", "Integração Asaas não configurada.");
...
export async function createStoreCouponAction(formData) {
  ...
  const { error } = await supabaseAdmin.from("barbearia_cupons").insert({
    barbearia_id: clinicId,
    codigo,
...
export async function toggleStoreCouponAction(formData) {
  ...
  const { error } = await supabaseAdmin.from("barbearia_cupons").update({ ativo }).eq("id", id).eq("barbearia_id", clinicId);
  if (error) redirectMessage("erro", error.message);
  revalidatePath("/dashboard/pedidos");
...
export async function openAbandonedCartRecoveryAction(formData) {
  ...
  const id = text(formData, "id", 80);
  const { data: cart } = await supabaseAdmin.from("barbearia_carrinhos_abandonados").select("id, telefone, token_recuperacao, quantidade_lembretes").eq("id", id).eq("barbearia_id", clinicId).maybeSingle();
  ...
  await supabaseAdmin.from("barbearia_carrinhos_abandonados").update({ lembrete_enviado_em: new Date().toISOString(), quantidade_lembretes: Number(cart.quantidade_lembretes || 0) + 1 }).eq("id", id);
