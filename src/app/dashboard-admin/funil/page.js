...
export default async function DashboardAdminFunilPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHero eyebrow="Funil comercial" title="Interesse, demonstração e conversão" description="Acompanhe os contatos gerados pelo site da NexaWi sem misturá-los ao CRM operacional de cada barbearia." />
      ...
      <section className="grid gap-3 rounded-[1.75rem] border border-neutral-200 bg-[#1c1c1c] p-5 text-white shadow-sm sm:grid-cols-2 xl:grid-cols-5">
        {[
          ...
          <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.06] p-4"><p className="text-xs font-bold uppercase tracking-wider text-white/45">{label}</p><strong className="mt-2 block text-2xl font-black">{value}</strong></div>
        ))}
      </section>
            ...
            <details key={lead.id} className="rounded-2xl border border-neutral-200 bg-[#fbfaf7] p-4">
              <summary className="cursor-pointer list-none">
                <div className="grid gap-3 sm:grid-cols-[1.2fr_1fr_0.7fr_0.7fr_auto] sm:items-center">
                  <div><p className="font-black">{lead.nome}</p><p className="text-xs text-neutral-500">{lead.barbearia_nome || "Barbearia não informada"}</p></div>
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-xs text-neutral-500">
                <span>{lead.barbeiros_qtd} {lead.barbeiros_qtd === 1 ? "barbeiro" : "barbeiros"}</span><span>{lead.utm_campaign ? `Campanha: ${lead.utm_campaign}` : "Sem campanha"}</span><span>{formatDate(lead.created_at)}</span>
              </div>
            </details>
          )) : <div className="rounded-2xl border border-dashed border-neutral-300 p-8 text-center text-sm text-neutral-500">Os contatos enviados pelo novo formulário aparecerão aqui.</div>}
        </div>
      </section>
      ...
      <section className="rounded-[1.75rem] border border-neutral-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[820px] border-separate border-spacing-y-2 text-left text-sm">
            <thead className="text-xs uppercase tracking-[0.14em] text-neutral-400"><tr><th className="px-3 py-2">Nome</th><th className="px-3 py-2">Origem</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Valor</th><th className="px-3 py-2">Próxima ação</th></tr></thead>
            <tbody>{recentCrm.map((item) => (
              <tr key={item.id} className="bg-[#fbfaf7]"><td className="rounded-l-2xl px-3 py-3 font-black">{item.nome}</td><td className="px-3 py-3">{item.origem || "Não informada"}</td><td className="px-3 py-3"><StatusPill tone={item.status === "convertido" ? "ok" : item.status === "perdido" ? "danger" : "accent"}>{item.status}</StatusPill></td><td className="px-3 py-3 font-bold">{formatMoney(item.valor_estimado)}</td><td className="rounded-r-2xl px-3 py-3 text-neutral-600">{formatDate(item.proxima_acao_em || item.created_at)}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </section>
