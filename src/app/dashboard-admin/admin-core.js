  ...
  return (
    <label className="block">
      ...
      <select
        ...
        className="mt-2 h-11 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm outline-none transition focus:border-[var(--nexawi-primary)]"
      >
        {children}
      </select>
    </label>
...
export function StatusPill({ children, tone = "neutral" }) {
  const tones = {
    ...
    accent: "bg-orange-50 text-[var(--nexawi-primary)]",
  };
  ...
  return (
    <article className={`min-w-0 rounded-[1.5rem] border p-5 shadow-sm ${dark ? "border-white/10 bg-[#1c1c1c] text-white" : "border-neutral-200 bg-white"}`}>
      <div className="flex min-w-0 items-start justify-between gap-4">
        ...
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${dark ? "bg-white/10 text-orange-300" : "bg-orange-50 text-[var(--nexawi-primary)]"}`}>
          <Icon size={21} />
        </div>
      </div>
...
export function PageHero({ eyebrow, title, description }) {
  return (
    <section className="relative min-w-0 overflow-hidden rounded-[2rem] bg-[#1c1c1c] p-6 text-white shadow-[0_30px_100px_rgba(28,28,28,0.24)] sm:p-7 lg:p-9">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_20%,var(--nexawi-primary-glow),transparent_24rem),radial-gradient(circle_at_88%_0%,rgba(255,255,255,0.12),transparent_24rem)]" />
...
export async function loadAdminAnalytics() {
  ...
  const [siteBookings, crm, appointments, asaasCharges, users, payments, clients, marketingLeads, marketingEvents] = await Promise.all([
    ...
    supabaseAdmin
      ...
      .order("created_at", { ascending: false })
      .limit(1200),
    supabaseAdmin.from("barbearia_usuarios").select("id, barbearia_id, email, ativo, aceito_em, created_at").order("created_at", { ascending: false }).limit(1200),
    supabaseAdmin.from("barbearia_pagamentos").select("id, barbearia_id, status, valor, valor_pago:valor, created_at, data_pagamento:pago_em").gte("created_at", last30Days).order("created_at", { ascending: false }).limit(1200),
    supabaseAdmin.from("barbearia_clientes").select("id, barbearia_id, status, origem, created_at").gte("created_at", last30Days).order("created_at", { ascending: false }).limit(1200),
    supabaseAdmin.from("barbearia_marketing_leads").select("id, nome, whatsapp, email, barbearia_nome, barbeiros_qtd, plano_interesse, origem, status, observacoes, utm_source, utm_medium, utm_campaign, pagina, created_at").order("created_at", { ascending: false }).limit(1200),
    supabaseAdmin.from("barbearia_marketing_eventos").select("id, event_name, session_id, lead_id, pagina, utm_source, utm_medium, utm_campaign, metadata, created_at").gte("created_at", last30Days).order("created_at", { ascending: false }).limit(3000),
  ]);
  ...
  return (
    <form action={upsertSystemPlanAction} className="rounded-[1.5rem] border border-neutral-200 bg-white p-5 shadow-sm">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Barbeiros" name="limite_barbeiros" type="number" defaultValue={plan?.limite_barbeiros ?? 3} />
