...
export async function GET(request) {
  ...
  if (cart.status === "ativo" && token === cart.token_recuperacao) {
    await supabaseAdmin.from("barbearia_carrinhos_abandonados").update({ status: "recuperado", ultima_interacao_em: new Date().toISOString() }).eq("id", cart.id);
  }
...
export async function POST(request) {
  ...
  const { data, error } = await supabaseAdmin
    .from("barbearia_carrinhos_abandonados")
    .upsert(row, { onConflict: "barbearia_id,sessao_token" })
