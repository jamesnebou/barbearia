...
function requestHash(request) {
  ...
  const salt = process.env.LEAD_HASH_SALT || process.env.BARBEARIA_SECRETS_KEY || "nexawi-public-lead";
...
export async function POST(request) {
  try {
    ...
    const barbeiros = Math.min(100, Math.max(1, Number.parseInt(body.barbers_count, 10) || 1));
    ...
    const { count } = await supabaseAdmin
      .from("barbearia_marketing_leads")
    ...
    const { data, error } = await supabaseAdmin.from("barbearia_marketing_leads").insert({
      nome,
      whatsapp,
      email,
      barbearia_nome: clean(body.barbershop_name, 120),
      barbeiros_qtd: barbeiros,
    ...
    await supabaseAdmin.from("barbearia_marketing_eventos").insert({
      ...
      utm_medium: clean(body.utm_medium),
      utm_campaign: clean(body.utm_campaign),
      metadata: { plano_interesse: plano, barbeiros_qtd: barbeiros },
      ip_hash: ipHash,
    });
