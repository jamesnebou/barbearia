...
const statusInfo = {
  ...
  confirmado: [CheckCircle2, "Pedido confirmado", "A barbearia recebeu seu pedido e iniciará a preparação."],
  em_separacao: [PackageCheck, "Em separação", "Os produtos estão sendo preparados."],
  pronto_retirada: [ShoppingBag, "Pronto para retirada", "Seu pedido já pode ser retirado na barbearia."],
  enviado: [Truck, "Pedido enviado", "Seu pedido saiu para entrega."],
  concluido: [CheckCircle2, "Pedido concluído", "Obrigado por comprar com a barbearia."],
...
export default async function PublicOrderPage({ params }) {
  ...
  const { data: clinic } = await supabaseAdmin.from("barbearias").select("id, nome, slug, telefone, endereco, cidade, estado, metadata").eq("slug", slug).maybeSingle();
  if (!clinic) notFound();

  const { data: order, error } = await supabaseAdmin
    .from("barbearia_pedidos")
    .select("id, numero, token_publico, status, pagamento_status, entrega_tipo, nome_cliente, telefone_cliente, subtotal, desconto, frete, total, cupom_codigo, forma_pagamento, invoice_url, expiracao_reserva_em, pago_em, created_at, cep, endereco, numero_endereco, complemento, bairro, cidade, estado, referencia_entrega, itens:barbearia_pedido_itens(id, nome_produto, sku, imagem_url, quantidade, valor_unitario, total)")
          ...
          <div className="grid gap-8 p-6 sm:p-9 lg:grid-cols-[1.15fr_0.85fr]">
            ...
            <aside>
              ...
              <div className="mt-4 rounded-2xl border border-neutral-200 bg-white p-5">
                ...
                <p className="mt-3 text-sm leading-6 text-neutral-600">{order.entrega_tipo === "entrega" ? [order.endereco, order.numero_endereco, order.complemento, order.bairro, order.cidade, order.estado, order.cep].filter(Boolean).join(" - ") : pickupAddress || "Confirme o endereço com a barbearia."}</p>
              </div>
