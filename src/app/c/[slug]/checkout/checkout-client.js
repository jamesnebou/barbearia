...
export function StoreCheckout({ slug, brandName, config, onlinePaymentAvailable, cartToken = "", query = {} }) {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      const token = cartToken || window.localStorage.getItem(`barbearia_cart_session_${slug}`) || window.crypto.randomUUID();
      setSessionToken(token);
      window.localStorage.setItem(`barbearia_cart_session_${slug}`, token);
          ...
          <section className="rounded-[2rem] border border-white/70 bg-white/72 p-6 shadow-[0_28px_80px_rgba(23,19,15,0.12)] backdrop-blur sm:p-8">
            ...
            {!items.length ? (
              ...
              <form action={createPublicStoreOrderAction} className="mt-8 space-y-7">
                ...
                {!canPay ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Nenhuma forma de pagamento está disponível. Fale com a barbearia.</div> : null}
