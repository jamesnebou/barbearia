...
function storageKey(slug) {
  return `barbearia_cart_${slug}`;
}

function getSessionToken(slug) {
  const key = `barbearia_cart_session_${slug}`;
...
function StoreProductCard({ product, onAdd, moving = false, index = 0 }) {
  ...
  return (
    <article className={cardClassName} data-store-index={index} style={moving ? { width: "17.5rem" } : undefined}>
      ...
      <div className="flex h-[340px] flex-col px-5 pb-5 pt-6">
        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--clinic-primary)]">{product.categoria || "Cuidados"}</p>
        <h3 className="mt-3 h-[4.25rem] overflow-hidden text-lg font-semibold leading-7 text-[#181510]" style={{ display: "-webkit-box", WebkitBoxOrient: "vertical", WebkitLineClamp: 2 }}>{product.nome}</h3>
        <p className="mt-3 h-14 overflow-hidden text-sm leading-6 text-neutral-600" style={{ display: "-webkit-box", WebkitBoxOrient: "vertical", WebkitLineClamp: 2 }}>{product.descricao || "Cuidado selecionado pela barbearia para complementar sua rotina."}</p>
...
export function PublicStorefront({ slug, products, recoveryToken = "", mode = "showcase" }) {
  ...
  useEffect(() => {
    const updateCartButtonVisibility = () => {
      setShowCartButton(window.scrollY > 160);
    };

    updateCartButtonVisibility();
    window.addEventListener("scroll", updateCartButtonVisibility, { passive: true });
    return () => window.removeEventListener("scroll", updateCartButtonVisibility);
  }, []);
    ...
    fetch(`/api/public/store/cart?slug=${encodeURIComponent(slug)}&token=${encodeURIComponent(recoveryToken)}`, { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        if (payload.sessionToken) {
          ...
          window.localStorage.setItem(`barbearia_cart_session_${slug}`, payload.sessionToken);
        }
        setOpen(true);
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    const speed = lowPower ? 28 : 42;
    const frameInterval = lowPower ? 32 : 16;
    let isVisible = true;
    ...
    const animate = (time) => {
      ...
      if (delta >= frameInterval) {
        lastTime = time;
        if (isVisible && !document.hidden && !reducedMotion && !showcasePointerRef.current.active && !showcaseHoverRef.current) {
  ...
  const shouldShowCartButton = showCartButton && totalQuantity > 0;
  ...
  const cartLayer = hydrated ? createPortal(
    <>
      <button
        ...
        className={`fixed right-20 top-4 z-[2147482900] sm:right-24 lg:right-8 flex h-12 min-w-12 items-center justify-center gap-3 rounded-full border border-white/15 bg-[#111]/95 px-0 text-sm font-black text-white shadow-[0_18px_55px_rgba(0,0,0,0.38)] backdrop-blur-xl transition duration-300 sm:h-14 sm:px-5 ${shouldShowCartButton ? "translate-y-0 opacity-100 hover:-translate-y-0.5" : "pointer-events-none -translate-y-4 opacity-0"}`}
        aria-label={`Abrir carrinho com ${totalQuantity} itens`}
        aria-hidden={!shouldShowCartButton}
        tabIndex={shouldShowCartButton ? 0 : -1}
      >
        <ShoppingBag size={20} />
    ...
    <>
      <section id="loja" className={mode === "full" ? "mx-auto min-h-[calc(100vh-5rem)] max-w-7xl px-5 py-16 sm:px-8 sm:py-20" : "public-section-warm mx-auto max-w-7xl px-5 py-24 sm:px-8"}>
        <div className="text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-neutral-600">{mode === "full" ? "Encontre todos os produtos disponíveis e escolha seus cuidados com tranquilidade." : "Escolha seus produtos, compre com segurança e retire na barbearia ou receba conforme a disponibilidade."}</p>
        </div>
