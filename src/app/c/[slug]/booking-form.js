...
export function PublicBookingForm({ slug, procedimentos, profissionais, query, timeZone = "America/Bahia" }) {
  ...
  const totals = useMemo(() => selectedProcedures.reduce((acc, item) => ({
    ...
    price: acc.price + activePrice(item),
  ...
  useEffect(() => {
    ...
    async function loadSlots() {
      ...
      procedimentoIds.forEach((id) => params.append("servico_ids", id));
      if (profissionalId) params.set("barbeiro_id", profissionalId);
      ...
      try {
        ...
        setSelectedSlotProfessional(nextSlots[0]?.barbeiro_id || "");
  ...
  function handleSlotChange(value) {
    ...
    setSelectedSlotProfessional(slot?.barbeiro_id || profissionalId || "");
  }
  ...
  return (
    <form action={createPublicBookingAction} className="rounded-[1.75rem] border border-white/70 bg-[#15120f] p-7 text-white shadow-[0_32px_90px_rgba(20,18,15,0.26)]">
      ...
      <input type="hidden" name="barbeiro_disponivel_id" value={selectedSlotProfessional} />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="relative block md:col-span-2">
          <span className="text-sm font-semibold text-white/75">Serviços</span>
          {procedimentoIds.map((id) => <input key={id} type="hidden" name="servico_ids" value={id} />)}
          <button type="button" onClick={() => setProceduresOpen((open) => !open)} className="mt-2 flex min-h-14 w-full items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-left text-sm text-white outline-none transition hover:border-white/25">
            <span>
              <strong className="block">{selectedProcedures.length ? `${selectedProcedures.length} serviço(s) selecionado(s)` : "Selecionar serviços"}</strong>
          ...
          {proceduresOpen ? (
            <div className="absolute left-0 right-0 top-full z-50 mt-3 overflow-hidden rounded-[1.5rem] border border-white/12 bg-[#211b18]/95 p-3 shadow-[0_28px_90px_rgba(0,0,0,0.45)] backdrop-blur-xl">
              <input value={procedureSearch} onChange={(event) => setProcedureSearch(event.target.value)} placeholder="Buscar serviço" className="h-11 w-full rounded-2xl border border-white/10 bg-white/10 px-4 text-sm text-white outline-none placeholder:text-white/35" />
              <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
                  ...
                  return (
                    <button key={item.id} type="button" onClick={() => toggleProcedure(item.id)} className={["flex w-full items-start justify-between gap-3 rounded-2xl border px-4 py-3 text-left text-sm transition", checked ? "border-[var(--clinic-accent)] bg-[var(--clinic-accent)]/18 text-white" : "border-white/10 bg-white/8 text-white/75 hover:border-white/25 hover:bg-white/12"].join(" ")}>
                      <span>
                        ...
                        <span className="mt-1 block text-xs text-white/55">{money(activePrice(item))} - {item.duracao_minutos || 60} min - {serviceLabel(item)}</span>
                      </span>
                ...
                {!filteredProcedures.length ? <p className="px-3 py-4 text-sm text-white/50">Nenhum serviço encontrado.</p> : null}
              </div>
            </div>
          ) : null}
        </div>
        ...
        <label className="block">
          <span className="text-sm font-semibold text-white/75">Barbeiro</span>
          <select name="barbeiro_id" value={profissionalId} onChange={(event) => setProfissionalId(event.target.value)} className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-white/10 px-4 text-sm text-white outline-none">
        ...
        <label className="block md:col-span-2">
          ...
          <select value={selectedSlot} onChange={(event) => handleSlotChange(event.target.value)} required disabled={loadingSlots || !slots.length} className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-white/10 px-4 text-sm text-white outline-none disabled:cursor-not-allowed disabled:opacity-55">
            ...
            {!loadingSlots && slots.map((slot) => (
              <option key={`${slot.value}-${slot.barbeiro_id}`} value={slot.value} className="text-neutral-950">
      ...
      <label className="mt-5 flex items-start gap-3 text-sm text-white/70">
        ...
        Aceito que meus dados sejam usados para contato, agendamento e atendimento, conforme política de privacidade da barbearia.
      </label>
