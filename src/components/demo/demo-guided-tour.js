...
const steps = [
  ...
  { title: "Agenda", description: "Confira horários, barbeiros, serviços, status e pagamentos no mesmo calendário.", href: "/dashboard/agenda", icon: CalendarDays },
  ...
  { title: "Site público", description: "Abra a vitrine que o cliente vê para escolher serviços, produtos e horários.", href: "/c/navalha-nobre-demo", icon: Globe2 },
];
    ...
    <div className="fixed inset-0 z-[90] flex items-end bg-black/55 p-4 backdrop-blur-sm sm:items-center sm:justify-center" role="dialog" aria-modal="true" aria-labelledby="demo-tour-title">
      <div className="w-full max-w-xl overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#111315] text-white shadow-[0_35px_120px_rgba(0,0,0,0.55)]">
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-sky-300">Tour da demonstração · {step + 1}/{steps.length}</p>
        ...
        <div className="p-6 sm:p-8">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-sky-400/12 text-sky-300"><Icon size={26} /></div>
          ...
          <div className="mt-7 flex gap-2">
            {steps.map((item, index) => <span key={item.title} className={`h-1.5 flex-1 rounded-full ${index <= step ? "bg-sky-400" : "bg-white/12"}`} />)}
          </div>
          <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
            ...
            <button onClick={next} className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-sky-400 px-6 text-sm font-black text-[#071019]">{step === steps.length - 1 ? "Abrir site e concluir" : "Abrir este módulo"}<ArrowRight size={16} /></button>
          </div>
        </div>
      </div>
    </div>
